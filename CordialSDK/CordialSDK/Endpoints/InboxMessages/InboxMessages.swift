...
class InboxMessages: NSObject, URLSessionDelegate {
    ...
    func getInboxMessagesPagination(pageRequest: PageRequest, contactKey: String, onSuccess: @escaping (_ response: InboxPage) -> Void, onFailure: @escaping (_ error: String) -> Void) {
            ...
            if let url = urlComponents.url {
                ...
                self.inboxMessagesURLSession.dataTask(with: request) { data, response, error in
                    ...
                    if let responseData = data, let httpResponse = response as? HTTPURLResponse {
                        ...
                        case 200:
                            self.parseResponsePaginationJSON(responseData: responseData, onSuccess: { response in
                                onSuccess(response)
                            }, onFailure: { error in
                                onFailure(error)
                            })
                        ...
                        case 401:
                            SDKSecurity.shared.updateJWTwithCallbacks(onSuccess: { response in
                                self.getInboxMessagesPagination(pageRequest: pageRequest, contactKey: contactKey, onSuccess: onSuccess, onFailure: onFailure)
                            }, onFailure: { error in
                                onFailure(error)
                            })
                        default:
    ...
    private func parseResponsePaginationJSON(responseData: Data, onSuccess: @escaping (_ response: InboxPage) -> Void, onFailure: @escaping (_ error: String) -> Void) {
        do {
