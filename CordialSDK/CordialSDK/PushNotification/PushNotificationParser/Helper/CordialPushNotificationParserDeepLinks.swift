...
class CordialPushNotificationParserDeepLinks {
    ...
    func getDeepLinkURLCurrentPayloadType(userInfo: [AnyHashable : Any]) -> URL? {
        if let system = userInfo["system"] as? [String: AnyObject], let deepLinkJSON = system["deepLink"] as? [String: AnyObject], let deepLinkURLString = deepLinkJSON["url"] as? String, let deepLinkURL = URL(string: deepLinkURLString) {
            return deepLinkURL
        } else if let system = userInfo["system"] as? [String: AnyObject], let deepLinkJSONString = system["deepLink"] as? String, let deepLinkJSONData = deepLinkJSONString.data(using: .utf8) {
            do {
    ...
    func getDeepLinkFallbackURLCurrentPayloadType(userInfo: [AnyHashable : Any]) -> URL? {
        if let system = userInfo["system"] as? [String: AnyObject], let deepLinkJSON = system["deepLink"] as? [String: AnyObject], let fallbackURLString = deepLinkJSON["fallbackUrl"] as? String, let fallbackURL = URL(string: fallbackURLString) {
            return fallbackURL
        } else if let system = userInfo["system"] as? [String: AnyObject], let deepLinkJSONString = system["deepLink"] as? String, let deepLinkJSONData = deepLinkJSONString.data(using: .utf8) {
            do {
